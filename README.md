...
本项目基于 **Excel + Power Query   Excel电源查询** 构建了一个自动化的销售数据监控仪表盘，用于分析月度成交金额、客户数、客单价及环比趋势，并支持按区域、省份、产品期数下钻。通过将新的每日销售明细和销售人员信息放入指定文件夹，即可一键刷新报表，无需手动修改公式。
...
2. 添加新月份数据
准备下个月的每日销售汇总文件，命名格式建议：YYYY-MM_每日销售成交汇总.xlsx（例如 2020-07_每日销售成交汇总.xlsx）。
...
3. 更新销售人员信息
将最新的 销售人员表.xlsx 替换 数据源 文件夹中的旧文件。
...
📄 示例数据说明
提供的示例数据为 2020年4月、5月、6月 的销售记录，以及截至 2020年8月1日 的销售人员信息。所有数据均为模拟脱敏数据，仅用于演示仪表盘功能。
